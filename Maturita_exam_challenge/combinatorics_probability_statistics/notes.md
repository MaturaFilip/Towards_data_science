...
- basics of statistics
  ...
  - not included (mean, median, modus, normal distribution etc.)
...
Addition principle / rule of sum = if there are n choices for one action, and m choices for another action and the ***_two actions cannot be done at the same time***, then there are n+m ways to choose one of these actions
...
- all elements that are in the set (A) and (B), or both
...
- all elements that are common to the sets (A) and (B)

$$
A \cap B
$$

![](https://learntube.pl/uploads/_/originals/l35-description-2.svg)
...
Probability of event A happen and B not happen

- The probability of event B not happen is 1 - P(B), so P(B′) = 1 - P(B)

$$
P(A∧B′)=P(A)⋅P(B′)=0,6⋅0,2=0,12\\[2ex]
$$

Probability of A and B happen at the same time (independent event)

$$
P(A∧B)=P(A)⋅P(B)=0,6⋅0,8=0,48\\[2ex]
$$

Probability that event A not happen and B happen

$$
P(A′∧B)=P(A)⋅P(B)=0,4⋅0,8=0,32\\[2ex]
$$

Probability that A and B not happen
...
![](https://learntube.pl/uploads/_/originals/l35-description-v2-1.svg)

Probability that event A or B happen

- So we can write it like:

$$
P(A∪B)=P(A∧B′)+P(A∧B)+P(A′∧B)\\[2ex]
P(A∪B)=0,12+0,48+0,32\\[2ex]
P(A∪B)=0,92\\[2ex]
$$

- or we can calculate that both event not happen and subtract this from 1
...
- but usually this method is used:
  
  - We have to subtract P(A∧B) otherwise the probability can be 1+, which is not possible

$$
P(A∪B)=P(A)+P(B)−P(A∧B)\\[2ex] 
P(A∪B)=0,6+0,8−0,48\\[2ex] 
P(A∪B)=0,92
$$
